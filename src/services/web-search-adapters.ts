...
function getRawMode(config: OpenHorseCLIConfig): string {
  return (
    config.webSearch?.provider
    || process.env.OPENHORSE_WEBSEARCH_PROVIDER
    || process.env.OPENHORSE_WEBSEARCH_MCP_PROVIDER
    || 'auto'
  ).trim().toLowerCase();
}

export function getWebSearchMode(config: OpenHorseCLIConfig): WebSearchMode {
...
function extractHits(payload: any): WebSearchHit[] {
  ...
  if (Array.isArray(payload)) {
    return payload.map(normalizeHit).filter(Boolean) as WebSearchHit[];
  }
...
const tavilyAdapter: WebSearchAdapter = {
  name: 'tavily',
  isConfigured() {
    return Boolean(process.env.TAVILY_API_KEY || process.env.OPENHORSE_WEBSEARCH_API_KEY);
  },
  async search(input) {
    const start = Date.now();
    const apiKey = process.env.TAVILY_API_KEY || process.env.OPENHORSE_WEBSEARCH_API_KEY;
...
const customAdapter: WebSearchAdapter = {
  name: 'custom',
  isConfigured() {
    return Boolean(process.env.OPENHORSE_WEBSEARCH_API || process.env.WEB_SEARCH_API);
  },
  async search(input) {
    const start = Date.now();
    const endpoint = process.env.OPENHORSE_WEBSEARCH_API || process.env.WEB_SEARCH_API;
    if (!endpoint) throw new Error('Custom search is not configured. Set OPENHORSE_WEBSEARCH_API.');

    const method = (process.env.OPENHORSE_WEBSEARCH_METHOD || process.env.WEB_METHOD || 'GET').toUpperCase();
    const queryParam = process.env.OPENHORSE_WEBSEARCH_QUERY_PARAM || process.env.WEB_QUERY_PARAM || 'q';
    const headers: Record<string, string> = { Accept: 'application/json' };
    const apiKey = process.env.OPENHORSE_WEBSEARCH_API_KEY || process.env.WEB_KEY;
    const authHeader = process.env.OPENHORSE_WEBSEARCH_API_KEY_HEADER || process.env.WEB_AUTH_HEADER;
...
export async function runWebSearchAdapters(
  ...
  if (chain.length === 0) {
    throw new Error(`No web search adapter is available for mode "${mode}".`);
  }
...
export function formatAdapterOutput(result: WebSearchAdapterOutput, query: string): string {
  const lines = [
    `Web search results for "${query}" via ${result.provider}:`,
    '',
    'Sources:',
  ];
