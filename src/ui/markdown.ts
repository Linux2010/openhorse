...
export function renderMarkdown(text: string, maxWidth = DEFAULT_MAX_WIDTH): string {
  ...
  // 如果包含表格，直接使用 fallback（marked-terminal 不支持表格）
  if (hasTable) {
...
/** 渲染 Markdown 表格 */
function renderTable(lines: string[], startIdx: number): { lines: string[]; nextIndex: number } {
  const result: string[] = [];
  let i = startIdx;
  ...
  for (let col = 0; col < colCount; col++) {
    ...
    for (const row of tableRows) {
      const cell = row[col] || '';
      const visualLen = cell.replace(/\x1b\[[0-9;]*m/g, '').length;
      maxWidth = Math.max(maxWidth, visualLen);
    }
    colWidths.push(Math.max(maxWidth, 3)); // 最小宽度 3
  }

  // 渲染表格
  const renderRow = (cells: string[], isHeader: boolean, aligns: typeof alignments) => {
    const parts: string[] = [];
    for (let col = 0; col < colWidths.length; col++) {
      ...
      if (align === 'center') {
        ...
        parts.push(' ' + ' '.repeat(leftPad) + content + ' '.repeat(rightPad) + ' ');
      } else if (align === 'right') {
        parts.push(' ' + ' '.repeat(padding) + content + ' ');
      } else {
        parts.push(' ' + content + ' '.repeat(padding) + ' ');
      }
    }
    return parts;
  };
  ...
  if (tableRows.length > 0) {
    const headerCells = renderRow(tableRows[0], true, alignments);
  ...
  for (let rowIdx = 1; rowIdx < tableRows.length; rowIdx++) {
    const cells = renderRow(tableRows[rowIdx], false, alignments);
    result.push('│' + cells.join('│') + '│');
    if (rowIdx < tableRows.length - 1) {
      result.push(rowSep);
    }
  }
