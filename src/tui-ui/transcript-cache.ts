...
export class TranscriptLayoutCache {
  ...
  get(id: string, revision: number, generation: number, width: number): StyledRow[] | null {
    if (generation !== this.lastGeneration || width !== this.lastWidth) {
      this.invalidate(generation, width);
      return null;
    }
  ...
  set(id: string, revision: number, rows: StyledRow[], generation: number, width: number): void {
    if (generation !== this.lastGeneration || width !== this.lastWidth) {
      this.invalidate(generation, width);
    }
    ...
    this.cache.set(id, { id, revision, rows });
  }

  /** Full invalidation on generation change or resize. */
  invalidate(generation: number, width: number): void {
    this.cache.clear();
